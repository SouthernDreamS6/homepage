...
# 基础 URL
url_template = "https://api.isthereanydeal.com/games/info/v2?id={game_id}&key={api_key}"
...
for game_id in game_ids:
    # 构建请求 URL
    url = url_template.format(game_id=game_id, api_key=api_key)
    ...
    if response.status_code == 200:
        ...
        # 创建以游戏 ID 命名的文件夹（如果不存在）
        directory = f"../public/itad/{game_id}"
        if not os.path.exists(directory):
            os.makedirs(directory)
        
        # 格式化 JSON 并保存到文件
        formatted_json_path = os.path.join(directory, "game_info.json")
        with open(formatted_json_path, 'w', encoding='utf-8') as json_file:
            json.dump(data, json_file, ensure_ascii=False, indent=4)
        
        print(f"数据已保存到 {formatted_json_path}")
    else:
        print(f"无法获取数据，状态码: {response.status_code}, 游戏 ID: {game_id}")
